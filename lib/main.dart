...
import 'package:flutter/material.dart';
    ...
    // Convert PdfPageImage to PNG bytes via ui.Image
    final ui.Image uiImage = await image.createImageDetached();
    final byteData = await uiImage.toByteData(format: ui.ImageByteFormat.png);
    final bytes = byteData?.buffer.asUint8List();
    image.dispose(); // Dispose PdfPageImage to free memory
      ...
      body: Center(
        child: LayoutBuilder(builder: (context, constraints) {
          ...
          return Stack(children: [
            // Back page (target)
            Positioned.fill(child: _fullPageWidget(target)),
            // Front page or turning animation on top
            Positioned.fill(
              child: _isFlipping
                  ? _buildPageTurn(_currentPage, target, width, height)
                  : _fullPageWidget(_currentPage),
  ...
  Widget _buildPageTurn(int fromPage, int toPage, double width, double height) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        // Back page slight parallax for depth (reduced)
        final backTransform = Matrix4.identity()
          ..setEntry(3, 2, 0.001)
          ..rotateY(_flipDirection ? 0.06 * (1 - p) : -0.06 * (1 - p));

        // Angle: hinge at spine (left for forward, right for backward)
        final maxAngle = pi * 0.45; // ~81 degrees for a softer open
        final angle = _flipDirection ? maxAngle * p : -maxAngle * p;

        return Stack(children: [
          // Back page (the page being revealed)
          Transform(
            transform: backTransform,
            alignment: Alignment.center,
            child: _fullPageWidget(toPage),
          ...
          // Rotating front page that hinges at the spine
          Align(
            alignment: Alignment.center,
            child: LayoutBuilder(builder: (context, c) {
              final hingeAlignment = _flipDirection ? Alignment.centerLeft : Alignment.centerRight;
              ...
              final pageTransform = Matrix4.identity()..setEntry(3, 2, 0.001);
              // rotate around vertical (Y) to open; no X-tilt for pure hinge opening
              pageTransform.rotateY(angle);
              ...
              return Transform(
                ...
                child: Stack(children: [
                  ...
                  // inner-side shading to simulate page thickness/fold
                  Positioned.fill(
                    child: IgnorePointer(
                      child: Container(
                        decoration: BoxDecoration(
                  ...
                  // underside shadow near the outer edge to emphasize lift
                  Positioned.fill(
                    child: IgnorePointer(
                      child: Align(
                        alignment: _flipDirection ? Alignment.centerRight : Alignment.centerLeft,
